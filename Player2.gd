...
func _ready():
	...
	g.add_point(0.1, Color(1, 0.9, 0.6, 1.0)) # 2: Pale Orange
	g.add_point(0.25, Color(1, 0.6, 0, 0.9))  # 3: Orange
	g.add_point(0.5, Color(1, 0.2, 0, 0.7))   # 4: Blood Orange
...
func _physics_process(delta):
	...
	if is_local and not has_meta("is_ai"):
		if GameState.current_mode == GameState.GameMode.LOCAL:
			# LOCAL: Player 2 uses Arrows only (Player 1 gets WASD)
			...
			# ONLINE: Your ship responds to BOTH WASD and Arrows
	...
	if GameState.current_mode == GameState.GameMode.ONLINE and multiplayer.get_unique_id() != 1:
		...
		# Still tick timers so the laser-ready indicator renders correctly
